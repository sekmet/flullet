...
import BuyItem from '@/components/markets/Buyitem';
...
export default function MyItem({
  ...
  const itemId = String(`${asset.item_id}`.replace(/"/g, ''));
  const sellingPrice = String(`${asset.price}`.replace(/"/g, ''));
  const sellingCurrency = String(`${asset.price_currency}`.replace(/"/g, ''));
  console.log(chainId);
  useEffect(() => {
        ...
        <div className="h-full shadow-md border-2 border-gray-200 rounded-md overflow-hidden">
          ...
          <div className="p-3">
            ...
            <div className="mt-3 flex items-center flex-wrap">
              <BuyItem contractAddress={address} itemId={itemId} />
            </div>
