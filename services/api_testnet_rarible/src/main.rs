...
pub fn get_all_assets(size: String, continue_token: String) -> String {
   log::info!("Get All Assets with Size:{} and Continuation Token:{} \n", size, continue_token);
...
#[marine]
pub fn get_collections(owner_address: String, size: String) -> String {
   log::info!("Get Collections of {} (Size:{})\n", owner_address, size);
...
#[marine]
pub fn get_assets_collection(collection_address: String, size: String) -> String {
   log::info!("Get Assets from Collection {} (Limit:{}) \n", collection_address, size);
   let url = format!("https://ethereum-api-staging.rarible.org/v0.1/nft/items/byCollection?collection={}&size={}", collection_address, size);
   let result = curl(vec![url]);
